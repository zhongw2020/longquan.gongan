...
export class ScrapComponent extends STComponentBase implements OnInit {
    ...
    let columns: OsharpSTColumn[] = [
      {
        title: '操作', fixed: 'left', width: 65, buttons: [{
          text: '操作', children: [
            { text: '编辑', icon: 'edit', acl: 'Root.Admin.OutStor.Scrap.Update', iif: row => row.Updatable, click: row => this.edit(row) },
            { text: '删除', icon: 'delete', type: 'del', acl: 'Root.Admin.OutStor.Scrap.Delete', iif: row => row.Deletable, click: row => this.delete(row) },
          ]
        }]
      },
